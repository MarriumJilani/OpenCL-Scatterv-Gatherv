...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < data_size; i++)
	{
		data2[i] = (int)(rand() % 2); //array of 0s and 1s
	}

	int* p = (int*)malloc(data_size * sizeof(int));
	for (int i = 0; i < data_size; i++)
	{
		p[i] = data[i]* data2[i];
	...
	printf("\nActual Dot Product of random data: %d\n ", s);
	...
	cl_mem sendbuf2 = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, data_size * sizeof(int), data2, &ret);
	...
	cl_mem localdotprod_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY, NUM_PROCESSES * sizeof(int), NULL, &ret);
	...
	ret = clEnqueueWriteBuffer(commandQueue, sendbuf2, CL_TRUE, 0, data_size * sizeof(int), data2, 0, NULL, NULL);;
	...
	// Create kernel
	cl_kernel kernel = clCreateKernel(program, "Gatherv", &ret);


	// Set arguments for kernel
	ret = clSetKernelArg(kernel, 0, sizeof(cl_mem), (void*)&sendbuf);
	ret = clSetKernelArg(kernel, 1, sizeof(cl_mem), (void*)&sendbuf2);
	ret = clSetKernelArg(kernel, 2, sizeof(cl_mem), (void*)&rcvbuf);
	ret = clSetKernelArg(kernel, 3, sizeof(cl_mem), (void*)&sendcountbuf);
	ret = clSetKernelArg(kernel, 4, sizeof(cl_mem), (void*)&displacementbuf);
	ret = clSetKernelArg(kernel, 5, sizeof(cl_mem), (void*)&localdotprod_buf);
	ret = clSetKernelArg(kernel, 6, sizeof(cl_mem), (void*)&coordinating_process_buffer);
	...
	int* arr = (int*)malloc(NUM_PROCESSES * sendcounts[0] * sizeof(int));

	ret = clEnqueueReadBuffer(commandQueue, localdotprod_buf, CL_TRUE, 0, NUM_PROCESSES * sizeof(int), local_sums, 0, NULL, NULL);
	ret = clEnqueueReadBuffer(commandQueue, rcvbuf, CL_TRUE, 0, NUM_PROCESSES * sendcounts[0] * sizeof(int), arr, 0, NULL, NULL);

	//printf("Received:%d\n",  local_sums);
	// Write result
	printf("\nGathering Arrays of products\n");
	//portion of arrays size using sendcount and displacements was randomly generated
	for (int i = 0; i < NUM_PROCESSES; ++i) 
	{
		printf("\nArray:%d \n", i);
	...
	for (int i = 0; i < NUM_PROCESSES; ++i) {
		...
		printf("%d:Local Dot products sum:%d\n",i, local_sums[i]);
		global_sum += local_sums[i];
	}

	printf("Dot Product:%d", global_sum);
	...
	ret = clReleaseMemObject(localdotprod_buf);
